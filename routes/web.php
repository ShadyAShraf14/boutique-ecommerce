...
// Payment callbacks (بيرجع من بوابة الدفع – ممكن يجي من برا، فبنسيبها بدون auth)
...
// Payment redirects (من عندنا للبوابة – لازم user)
...
// Checkout (protected: لازم Login)
Route::middleware('auth')->group(function () {

    Route::get('/checkout', [CheckoutController::class, 'index'])
        ->name('frontend.checkout.index');

    // لو حابة تخلي الكوبون من جوه الـ checkout بس، دخّليهم هنا
...
/*
|--------------------------------------------------------------------------
| 5) Auth Routes (Login / Register / Forgot Password...)
|--------------------------------------------------------------------------
*/
...
/*
|--------------------------------------------------------------------------
| 6) Admin Auth + Admin Routes (Roles & Permissions)
|--------------------------------------------------------------------------
*/
...
Route::prefix('admin')->name('admin.')->group(function () {
    ...
    Route::middleware(['auth:admin'])->group(function () {
        ...
        */
        ...
        Route::get('/supervisors/{supervisor}/edit', [SupervisorController::class, 'edit'])
        ...
        */
        ...
        Route::get('/shipping_companies', [ShippingCompanyController::class, 'index'])
            ->name('shipping_companies.index');

        Route::get('/shipping_companies/create', [ShippingCompanyController::class, 'create'])
            ->name('shipping_companies.create');

        Route::post('/shipping_companies', [ShippingCompanyController::class, 'store'])
            ->name('shipping_companies.store');

        Route::get('/shipping_companies/{shipping_company}/edit', [ShippingCompanyController::class, 'edit'])
            ->name('shipping_companies.edit');

        Route::put('/shipping_companies/{shipping_company}', [ShippingCompanyController::class, 'update'])
            ->name('shipping_companies.update');

        Route::get('/shipping_companies/{shipping_company}', [ShippingCompanyController::class, 'show'])
            ->name('shipping_companies.show');

        Route::delete('/shipping_companies/{shipping_company}', [ShippingCompanyController::class, 'destroy'])
            ->name('shipping_companies.destroy');
...
Route::middleware(['web'])->group(function () {
    ...
    Route::get(
        '/my-notifications',
        [FrontNotificationController::class, 'index']
    )->name('front.notifications.index');

    Route::get(
        '/my-notifications/{id}/open',
        [FrontNotificationController::class, 'open']
    )->name('front.notifications.open');

    Route::post(
        '/my-notifications/read-all',
        [FrontNotificationController::class, 'markAllAsRead']
...
Route::middleware(['auth', 'role:customer'])
    ...
    ->group(function () {
        ...
        Route::get(
            ...
            [CustomerInvoiceController::class, 'download']
        )->name('invoices.download');
...
Route::get('/login', function () {
    return view('auth.login');    // ده الملف اللي فيه الديزاين الجديد
